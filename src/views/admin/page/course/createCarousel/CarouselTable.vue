...
<script>
    ...
    import {createCarousel, updateCarousel} from "../../../../../service/api";
    ...
    export default {
        ...
        created(){
            //  判断是否存在传递的 carousel，存在则是修改界面调用表单， 否则为新建界面调用表单
            if (this.carousel){
                //  更新修改carousel 的逻辑  把传递的参数复制给表单对应的数据
                this.form.course_id = this.carousel.course_id;
            }else{
                //   创建carousel 的逻辑
        ...
        methods:{
            ...
            onSubmit(){
                ...
                if (this.carousel){
                    alert('是否更新轮播图!');
                    updateCarousel(this.carousel.id, this.form).then(res=>{
                        this.cancelShowData();
                        console.log(res);
                        this.reload();
                    }).catch(err=>{
